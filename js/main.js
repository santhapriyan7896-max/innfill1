...
(function initTestimonials() {
  ...
  const goTo = (idx) => {
    current = idx;
    // Scroll within the track container, not the page
    const card = cards[idx];
    const trackLeft = track.getBoundingClientRect().left;
    const cardLeft  = card.getBoundingClientRect().left;
    track.scrollLeft += (cardLeft - trackLeft);
    dots.forEach((d, i) => d.classList.toggle('active', i === idx));
  };

  dots.forEach((d, i) => d.addEventListener('click', () => goTo(i)));
  let auto = setInterval(() => goTo((current + 1) % cards.length), 4500);
  track.addEventListener('mouseenter', () => clearInterval(auto));
  track.addEventListener('mouseleave', () => { auto = setInterval(() => goTo((current + 1) % cards.length), 4500); });
})();
...
(function initCursor() {
  ...
  // ── State ──
  let mx = -300, my = -300;   // actual mouse
  let hx = -300, hy = -300;   // lerped head position
  let isHovering = false;
  let isOnPage   = false;

  // Particle pool
  const particles = [];
  // Ripple pool (click bursts)
  const ripples   = [];

  // Colors cycling
  const COLORS = ['#3B82F6', '#06B6D4', '#8B5CF6', '#60A5FA', '#38BDF8'];
  ...
  document.addEventListener('mousemove', e => {
    ...
    if (Math.random() < 0.55) spawnParticle(mx, my);
  });
  document.addEventListener('mouseleave', () => { isOnPage = false; });

  // Click ripple burst
  document.addEventListener('mousedown', e => {
    for (let i = 0; i < 12; i++) spawnParticle(e.clientX, e.clientY, true);
    ripples.push({ x: e.clientX, y: e.clientY, r: 0, alpha: 0.9 });
  });

  // Hover state
  const hoverSel = 'a, button, [role="button"], .faq-question, .testimonial-dot, .feature-card, .testimonial-card, .btn';
  ...
  function spawnParticle(x, y, burst = false) {
    ...
    const speed  = burst ? (3 + Math.random() * 6) : (0.8 + Math.random() * 2.2);
    const color  = COLORS[Math.floor(Math.random() * COLORS.length)];
    particles.push({
      x, y,
      vx: Math.cos(angle) * speed * (burst ? 1 : 0.4),
      vy: Math.sin(angle) * speed * (burst ? 1 : 0.35) - (burst ? 0 : 0.8),
      r:  burst ? (2.5 + Math.random() * 3) : (1.5 + Math.random() * 2.5),
      alpha: 0.9 + Math.random() * 0.1,
      decay: burst ? (0.03 + Math.random() * 0.03) : (0.025 + Math.random() * 0.025),
      color,
      glow: burst ? 14 : 8,
    });
  }
  ...
  function drawHead(x, y) {
    if (!isOnPage) return;
    const r  = isHovering ? 14 : 7;
    const c  = isHovering ? '#06B6D4' : '#3B82F6';

    // Outer glow ring
    ctx.save();
    ctx.beginPath();
    ctx.arc(x, y, isHovering ? 22 : 18, 0, Math.PI * 2);
    ctx.strokeStyle = isHovering ? 'rgba(6,182,212,0.22)' : 'rgba(59,130,246,0.18)';
    ...
    // Cross-hair lines when hovering
    if (isHovering) {
      ctx.save();
  ...
  function loop() {
    ...
    const lerpSpeed = isHovering ? 0.18 : 0.28;
    hx += (mx - hx) * lerpSpeed;
    hy += (my - hy) * lerpSpeed;
